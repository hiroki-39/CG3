...
#pragma endregion
	...
	while (true)
	{
		...
		//3Dオブジェクト編集用パネル
		if (ImGui::CollapsingHeader("3D Object"))
		{
			ImGui::Text("Plane.obj");

			// Transform 編集 (位置・回転・スケール)
			Vector3 pos3 = transformPlaneObj.translate;
			if (ImGui::DragFloat3("Position (X,Y,Z)", &pos3.x, 0.1f))
			{
				transformPlaneObj.translate = pos3;
			}

			// 回転：ImGui の SliderAngle は内部でラジアンを扱う（ラジアン格納前提）
			float rx = transformPlaneObj.rotation.x;
			float ry = transformPlaneObj.rotation.y;
			float rz = transformPlaneObj.rotation.z;
			if (ImGui::SliderAngle("Rotation X", &rx, -360.0f, 360.0f)) transformPlaneObj.rotation.x = rx;
			if (ImGui::SliderAngle("Rotation Y", &ry, -360.0f, 360.0f)) transformPlaneObj.rotation.y = ry;
			if (ImGui::SliderAngle("Rotation Z", &rz, -360.0f, 360.0f)) transformPlaneObj.rotation.z = rz;

			Vector3 scale3 = transformPlaneObj.scale;
			if (ImGui::DragFloat3("Scale (X,Y,Z)", &scale3.x, 0.01f, 0.001f, 100.0f)) transformPlaneObj.scale = scale3;
			...
			// ライティングの有効化
			ImGui::Checkbox("Enable Lighting", &materialDataPlaneObj->enableLighting);
			// ライティングタイプ（インデックス）
			const char* lightingNames[] = { "None", "Lambert", "Phong", "Custom" };
			int lightingCount = _countof(lightingNames);
			if (ImGui::Combo("Lighting", &materialDataPlaneObj->selectLightings, lightingNames, lightingCount))
			{
				// 選択変更時の処理があればここに
			}
			...
			// テクスチャ切替（利用可能なテクスチャ群から選択）
			static int selected3DTex = 0;
			// textureNames, textureIndices は main 内で定義済み（最初のシーン初期化部）
			//if (ImGui::Combo("3D Texture", &selected3DTex, textureNames.data(), (int)textureNames.size()))
			//{
			//	// SRV ハンドルを切り替える（描画時に使用される handle へ反映）
			//	textureSrvHandleGPU2 = TextureManager::GetInstance()->GetSrvHandleGPU(textureIndices[selected3DTex]);
			//}
			...
			ImGui::DragFloat("Light Intensity", &directionalLightData->intensity, 0.01f, 0.0f, 100.0f);

			// UI で変更があった場合、即座に WVP / World を更新して描画に反映させる
			//（変数名はスコープ内の既存変数を利用）
			{
				// カメラ行列を再取得
				Matrix4x4 worldMatrix = transformPlaneObj.GetWorldMatrix();
				Matrix4x4 cameraMatrixLoc = camera.GetWorldMatrix();
				Matrix4x4 viewMatrix = Matrix4x4::Inverse(cameraMatrixLoc);
				Matrix4x4 projectionMatrix = Matrix4x4::Perspective(0.45f, float(winApp->kClientWidth) / float(winApp->kClientHeight), 0.1f, 100.0f);
				Matrix4x4 wvp = Matrix4x4::Multiply(worldMatrix, Matrix4x4::Multiply(viewMatrix, projectionMatrix));

				// GPU へ書き込み
				transformationMatrixDataPlaneObj->World = worldMatrix;
				transformationMatrixDataPlaneObj->WVP = wvp;

				// ライトの方向ベクトルは正規化しておく
				directionalLightData->direction = directionalLightData->direction.Normalize();

				// マテリアルやライトの CBuffer 等はすでに GPU リソースへマップ済みなので、
				// ここで materialDataPlaneObj や directionalLightData を直接変更すれば描画時に反映されます。
			}
		}
		...
		//dxCommon->GetCommandList()->SetGraphicsRootConstantBufferView(2, texManager->GetSrvHandleGPU(textureIndices[selectedModel]).ptr);
