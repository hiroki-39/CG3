...
#pragma region ゲーム固有の初期化

    // カメラ作成（ゲーム固有）
    camera = new Camera();
    camera->SetTranslate({ 0.0f, 6.0f, -20.0f });
    camera->SetRotation({ 0.3f, 0.0f, 0.0f });
    // フレームワーク側で作成した object3dCommon_ を使ってデフォルトカメラを設定
    if (object3dCommon_)
    {
    ...
    // インスタンシング配列などのポインタは renderer 初期化後に取得する
    ...
    ParticleManager::GetInstance()->SetupRendererFromAsset(particleRenderer, "quad", dxCommon_, srvManager_, kNumMaxInstance);
    ...
    // スプライト
    {
        Sprite* s = new Sprite();
        // テクスチャはuvChecker
        s->Initialize(spriteCommon_, uvCheckerTex);
        // 初期位置を(100,100)
    ...
    // モデル
    {
        ...
        terrain->Initialize(object3dCommon_);
    ...
    // mp3を読み込む（SoundManager はフレームワーク側で初期化済み）
...
void Application::Update()
{
    ...
    // ImGui の受付は FrameworkUpdate で既に Begin 済み
    // ここで ImGui ウィンドウ生成 → End() まで行う
...
void Application::Draw()
{
    ...
    // Note:
    // - dxCommon_->PreDraw()/PostDraw() と ImGui の描画は KHFramework が行う（FrameworkDrawBegin/End）。
    // - ここではゲーム固有の描画呼び出しのみ行う。
    ...
    if (spriteCommon_)
    {
        spriteCommon_->SetCommonDrawSetting();
    }
